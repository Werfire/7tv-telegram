...
Start by typing `@s7ntvbot` in any chat. You can specify optional number for limit as prefix and then optional @CHANNEL_NAME to search emotes from some channel with its aliases. You can also make search exact match and case-sensitive by quoting emote name (case-insensitive available with single quotes) and/or add caption to sent emote with emote name by prefixing search with '*'. Non-ASCII (e.g. cyrillic) emote names will also work.
